...
import { Check, Pencil, RefreshCw, ArrowRight, TrendingDown, AlertTriangle } from 'lucide-react';

export function PricingCalculatorV2({ accounts = [], borrowerData = {}, onSelectChart }) {
  // Configuration state
  ...
  return (
    <div className="p-6 bg-[#fafafa] min-h-screen">
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1e1b4b]">Scenarios 2</h1>
        <p className="text-sm text-[#6b7280]">Compact comparison view</p>
      </div>
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-[#e5e7eb] rounded-xl overflow-hidden">
              ...
              <div className="grid grid-cols-3">
                <div className="px-4 py-3 bg-white border-b-2 border-[#e5e7eb]"></div>
                <div className="px-4 py-3 bg-[#fef6f0] border-b-2 border-[#f5e6d8] text-center">
                  <span className="text-base font-bold text-[#92400e]">Current</span>
                  <p className="text-xs text-[#b45309]">What you pay now</p>
                </div>
                <div className="px-4 py-3 bg-[#f5f3ff] border-b-2 border-[#e9e5ff] text-center">
                  <span className="text-base font-bold text-[#432c9e]">GoodLeap</span>
              ...
              <div className="divide-y divide-[#e5e7eb]">
                ...
                <div className="grid grid-cols-3 items-center min-h-[48px]">
                  <div className="px-4 py-3 text-base text-[#111827]">Principal & Interest</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(currentPI)}</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(proposedPI)}</div>
                </div>

                {/* Subordinate Lien */}
                <div className="grid grid-cols-3 items-center min-h-[48px]">
                  <div className="px-4 py-3 text-base text-[#111827]">Subordinate Lien</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(monthlySubordinateLien)}</div>
                  <div className="px-4 py-3 text-center text-base font-bold text-green-600">$0 ✓</div>
                </div>

                {/* Taxes - only when escrows enabled */}
                {escrowsEnabled && (
                  <div className="grid grid-cols-3 items-center min-h-[48px]">
                    <div className="px-4 py-3 text-base text-[#111827] pl-8">Taxes</div>
                    <div className="px-4 py-3 flex justify-center">
                      <input
                        type="text"
                        value={monthlyTaxes.toLocaleString()}
                        onChange={(e) => setMonthlyTaxes(parseInt(e.target.value.replace(/,/g, '')) || 0)}
                        className="w-20 px-2 py-1 border-2 border-[#e5e7eb] rounded-lg text-base font-semibold text-[#0f172a] text-center focus:border-[#432c9e] focus:outline-none"
                      />
                    </div>
                    <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(monthlyTaxes)}</div>
                  </div>
                )}

                {/* Insurance - only when escrows enabled */}
                {escrowsEnabled && (
                  <div className="grid grid-cols-3 items-center min-h-[48px]">
                    <div className="px-4 py-3 text-base text-[#111827] pl-8">Insurance</div>
                    <div className="px-4 py-3 flex justify-center">
                      <input
                        type="text"
                    ...
                    <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(monthlyInsurance)}</div>
                  </div>
                )}

                {/* Mortgage Insurance */}
                <div className="grid grid-cols-3 items-center min-h-[48px]">
                  <div className="px-4 py-3 text-base text-[#111827] pl-8">Mortgage Insurance</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(currentMI)}</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(proposedMI)}</div>
                </div>

                {/* Mortgage Total */}
                <div className="grid grid-cols-3 items-center min-h-[52px] bg-gradient-to-r from-[#fef6f0] via-white to-[#f5f3ff]">
                  <div className="px-4 py-3 text-base font-bold text-[#111827]">Mortgage Total</div>
                  <div className="px-4 py-3 text-center text-base font-bold text-[#92400e]">{formatCurrency(currentMortgageTotal)}</div>
                  <div className="px-4 py-3 text-center text-base font-bold text-[#432c9e]">{formatCurrency(proposedMortgageTotal)}</div>
                </div>

                {/* Monthly Debts (Paid Off) */}
                <div className="grid grid-cols-3 items-center min-h-[48px]">
                  <div className="px-4 py-3 text-base text-[#111827]">Monthly Debts (Paid Off)</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-rose-600">{formatCurrency(monthlyDebtsPaid)}</div>
                  <div className="px-4 py-3 text-center text-base font-bold text-green-600">$0 ✓</div>
                </div>

                {/* Monthly Debts (NOT Paid) */}
                <div className="grid grid-cols-3 items-center min-h-[48px]">
                  <div className="px-4 py-3 text-base text-[#111827]">Monthly Debts (NOT Paid)</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(monthlyDebtsNotPaid)}</div>
                  <div className="px-4 py-3 text-center text-base font-semibold text-[#0f172a]">{formatCurrency(monthlyDebtsNotPaid)}</div>
                </div>

                {/* Total Monthly */}
                <div className="grid grid-cols-3 items-center min-h-[56px] bg-gradient-to-r from-[#fef6f0] via-white to-[#f5f3ff] border-t-2 border-[#e5e7eb]">
                  <div className="px-4 py-3 text-lg font-bold text-[#111827]">TOTAL MONTHLY</div>
                  <div className="px-4 py-3 text-center text-xl font-bold text-[#92400e]">{formatCurrency(currentTotal)}</div>
                  <div className="px-4 py-3 text-center text-xl font-bold text-[#432c9e]">{formatCurrency(proposedTotal)}</div>
                </div>
              </div>
            </div>
